...
const ISSUE_THINKING_EFFORT_OPTIONS = {
  claude_local: [
    { value: "", label: t("agentConfig.default") },
...
function slugifyDocumentKey(input: string) {
  const slug = input
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  return slug || t("issueDetail.documentFallback");
}
...
const statuses = [
  { value: "backlog", label: t("status.backlog"), color: issueStatusText.backlog ?? issueStatusTextDefault },
  { value: "todo", label: t("status.todo"), color: issueStatusText.todo ?? issueStatusTextDefault },
  { value: "in_progress", label: t("status.inProgress"), color: issueStatusText.in_progress ?? issueStatusTextDefault },
  { value: "in_review", label: t("status.inReview"), color: issueStatusText.in_review ?? issueStatusTextDefault },
  { value: "done", label: t("status.done"), color: issueStatusText.done ?? issueStatusTextDefault },
];
...
const EXECUTION_WORKSPACE_MODES = [
  ...
  { value: "isolated_workspace", label: t("issueWorkspace.newIsolated") },
  { value: "reuse_existing", label: t("issueWorkspace.reuseExisting") },
] as const;
...
export function NewIssueDialog() {
  ...
  const createIssue = useMutation({
    ...
    onSuccess: ({ issue, companyId, failures }) => {
      ...
      if (failures.length > 0) {
        ...
        pushToast({
          title: `${issueRef}: ${t("dialogs.creatingIssue")}`,
  ...
  const uploadDescriptionImage = useMutation({
    mutationFn: async (file: File) => {
      if (!effectiveCompanyId) throw new Error(t("project.noCompanySelected"));
  ...
  const createIssueErrorMessage =
    createIssue.error instanceof Error ? createIssue.error.message : t("dialogs.failedCreateIssue");
      ...
      >
        ...
        <div className="px-4 pt-4 pb-2 shrink-0">
          <textarea
            ...
            placeholder={t("dialogs.issueTitle")}
            rows={1}
            value={title}
            onChange={(e) => {
          ...
          <div className="overflow-x-auto overscroll-x-contain">
            <div className="inline-flex items-center gap-2 text-sm text-muted-foreground flex-wrap sm:flex-nowrap sm:min-w-max">
              ...
              <InlineEntitySelector
                ...
                emptyMessage={t("dialogs.noProjectsFound")}
        ...
        {currentProject && currentProjectSupportsExecutionWorkspace && (
          <div className="px-4 py-3 shrink-0 space-y-2">
            <div className="space-y-1.5">
              <div className="text-xs font-medium">{t("dialogs.executionWorkspace")}</div>
              <div className="text-[11px] text-muted-foreground">
                {t("dialogs.executionWorkspaceHelp")}
              </div>
              <select
              ...
              {executionWorkspaceMode === "reuse_existing" && (
                ...
                >
                  <option value="">{t("dialogs.chooseExistingWorkspace")}</option>
          ...
          <div className="px-4 pb-2 shrink-0">
            ...
            {assigneeOptionsOpen && (
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("dialogs.model")}</div>
                  <InlineEntitySelector
                    value={assigneeModelOverride}
                    options={modelOverrideOptions}
                    placeholder={t("dialogs.defaultModel")}
                    disablePortal
                    noneLabel={t("dialogs.defaultModel")}
                    searchPlaceholder={t("agentConfig.searchModels")}
                    emptyMessage={t("dialogs.noModelsFound")}
                    onChange={setAssigneeModelOverride}
                  />
                </div>
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("dialogs.thinkingEffort")}</div>
                ...
                {assigneeAdapterType === "claude_local" && (
                  <div className="flex items-center justify-between rounded-md border border-border px-2 py-1.5">
                    <div className="text-xs text-muted-foreground">{t("dialogs.enableChrome")}</div>
                    <button
                      data-slot="toggle"
                      className={cn(
            ...
            <div className="mt-4 space-y-3 rounded-lg border border-border/70 p-3">
              {stagedDocuments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("dialogs.documents")}</div>
                  <div className="space-y-2">
                    {stagedDocuments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          disabled={createIssue.isPending}
                          title={t("dialogs.removeDocument")}
                        >
              ...
              {stagedAttachments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("dialogs.attachments")}</div>
                  <div className="space-y-2">
                    {stagedAttachments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          onClick={() => removeStagedFile(file.id)}
                          disabled={createIssue.isPending}
                          title={t("dialogs.removeAttachment")}
                        >
                          <X className="h-3.5 w-3.5" />
                        </Button>
                      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                ...
                <span>{createIssue.isPending ? t("dialogs.creating") : t("dialogs.createIssue")}</span>
              </span>
            </Button>
          </div>
        </div>
