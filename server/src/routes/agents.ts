...
import { agents as agentsTable, approvals, companies, heartbeatRuns, issues as issuesTable } from "@paperclipai/db";
  ...
  async function materializeDefaultInstructionsBundleForNewAgent<T extends {
    ...
    const files = promptTemplate.trim().length === 0
      ? await loadDefaultAgentInstructionsBundle(resolveDefaultAgentInstructionsBundleRole(agent.role), {
        name: agent.name,
        role: agent.role,
        title: asNonEmptyString((agent as { title?: unknown }).title),
        capabilities: asNonEmptyString((agent as { capabilities?: unknown }).capabilities),
      })
