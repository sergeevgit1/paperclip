...
import { t } from "@/i18n";
import { displayPriority, displayStatus } from "@/lib/display";
...
const quickFilterPresets = [
  { label: t("issues.preset.all"), statuses: [] as string[] },
  { label: t("issues.preset.active"), statuses: ["todo", "in_progress", "in_review", "blocked"] },
  { label: t("issues.preset.backlog"), statuses: ["backlog"] },
  { label: t("issues.preset.done"), statuses: ["done", "cancelled"] },
];
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "status") {
      ...
      return statusOrder
        ...
        .map((s) => ({ key: s, label: displayStatus(s), items: groups[s]! }));
    }
    if (viewState.groupBy === "priority") {
      const groups = groupBy(filtered, (i) => i.priority);
      return priorityOrder
        .filter((p) => groups[p]?.length)
        .map((p) => ({ key: p, label: displayPriority(p), items: groups[p]! }));
    }
    // assignee
  ...
  return (
    <div className="space-y-4">
              ...
              <div className="p-3 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                  ...
                  <div className="space-y-1">
                     <span className="text-xs text-muted-foreground">{t("issues.sort.status")}</span>
                    <div className="space-y-0.5">
                      {statusOrder.map((s) => (
                        <label key={s} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                          ...
                          />
                          <StatusIcon status={s} />
                          <span className="text-sm">{displayStatus(s)}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                      ...
                      <div className="space-y-0.5">
                        {priorityOrder.map((p) => (
                          <label key={p} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                            ...
                            <span className="text-sm">{displayPriority(p)}</span>
                          </label>
                        ))}
                      </div>
                    </div>
                    ...
                    {projects && projects.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t("issues.project")}</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
            ...
            <Popover>
              ...
              <PopoverContent align="end" className="w-48 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["status", t("issues.sort.status")],
                    ["priority", t("issues.sort.priority")],
                    ["title", t("issues.sort.title")],
                    ["created", t("issues.sort.created")],
                    ["updated", t("issues.sort.updated")],
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              ...
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["status", t("issues.group.status")],
                    ["priority", t("issues.group.priority")],
                    ["assignee", t("issues.group.assignee")],
                    ["none", t("issues.group.none")],
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("issues.emptyFiltered")}
                        ...
                        >
                          <input
                            className="mb-1 w-full border-b border-border bg-transparent px-2 py-1.5 text-xs outline-none placeholder:text-muted-foreground/50"
                             placeholder={t("commentThread.searchAssignees")}
