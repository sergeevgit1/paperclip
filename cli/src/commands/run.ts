...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  if (!configExists(configPath)) {
    if ((!process.stdin.isTTY || !process.stdout.isTTY) && !opts.yes) {
      ...
      p.log.message(
        `Run ${pc.cyan("paperclipai onboard")} once, retry ${pc.cyan("paperclipai run")}, or use ${pc.cyan("paperclipai run --yes")}.`,
      );
      process.exit(1);
    }

    p.log.step("No config found. Starting onboarding...");
    await onboard({ config: configPath, invokedByRun: true, yes: opts.yes ?? false });
  }
